...
2.  在概览页单击**新手引导**，然后在新手引导页面查看并保存 license。 
    ...
    ![新手引导](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/1332370/156750227156979_zh-CN.png)

3.  在概览页单击**下载探针**，然后根据您的主机规则选择下载探针的类型（Linux 32 位或 Linux 64 位）。 

    ![下载探针](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/1332370/156750227156980_zh-CN.png)
...
5.  执行以下命令启动探针。 
    ...
    **说明：** 
    ...
    -   若出现权限不足的情况，请切换至 admin 用户或 root 用户再执行 start.sh，还可以通过执行 `sudo -u admin sh start.sh`或`sudo sh start.sh`启动探针。
    例如：
